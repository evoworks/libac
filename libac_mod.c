...
///////////////////////////////////////////////
int global_control[10], global_np, global_np0, global_posG[100], global_ig, global_count, global_time, global_rate[20], global_stop, global_iteration;
int global_pose[3][500000], global_site[3][500000], global_initial[500000], global_final[5000000], global_x[200000], global_temp[3][500000];
double global_y[200000], global_parameters[200000], global_c0[100], global_omega0[100], global_kappa0[100], global_c, global_c1[20], global_c2[200], global_t[5000], global_z[8][500000], global_Pi[200], global_z1[8][500000];
double global_like[8][500000], global_like1[8][500000], global_posterior[8][500000], global_posterior1[8][500000], global_posterior2[8][500000], global_sum, global_freqt[8][64], global_count2, global_order[500000];
double global_temp0, global_lnL, global_lnLk[500000], global_lnL_max, global_lnL_min, global_kappa, global_omega, global_c_lnL[2];
int global_random, global_freq[8][500000], global_k, global_seed[2][500000], global_w[200], global_power[10000], global_model[3], global_Mgene, global_Pi1[200];
///////////////////////////////////////////////
...
int InitializeCodon (FILE *fout, double space[], int global_time);
...
FILE *fout, *frub, *flnf, *frst, *frst1, *finitials, *fparameter, *frandom, *fpower, *fgap, *fp;
char *ratef="rates";
...
////////////////////////////////////////////
  ...
  fprintf(frandom, "Pi:\n");
   for (i=0;i<global_k;i++){
      FOR(k,64) {
        fprintf(frandom,"%12.8f",com.piG[i][k]);
  /*,GeneticCode[com.icode][k]==-1?0: I moved this from in front of com.piG  kathy*/
        if((k+1)%4==0) FPN(frandom); /* disabled this to see if I can get the 0's in the right place*/
      }
      FPN(frandom);
     }

/*   fprintf(frandom, "%f ", com.piG[i][64]); I though of adding this ??? kathy*/
   }
...
/*      for (global_ig=0;global_ig<global_k;global_ig++)
      {
          for (j=0;j<global_k;j++)
          fprintf(fparameter, "%d ", global_Pi1[10+10*global_ig+j]);
          fprintf(fparameter, "\n");
      }
      fprintf(fparameter, "\n");
      for (j=0;j<global_k;j++)
      fprintf(fparameter, "%d ", global_Pi1[j]);
      fprintf(fparameter, "\n");
*/
      for (j=0;j<global_k;j++)
      {
...
if (global_k==3)
{
   ...
   {
   //for (i=0;i<global_k;i++)
   //fprintf(fparameter, "%f ", global_Pi[i]);
...
/*
   fprintf(fparameter, "\n ini site group  ");
   for (i=0;i<com.ls;i++)
   fprintf(fparameter, " %d", global_initial[i]+1);

   fprintf(fparameter, "\n final site group");
   for (i=0;i<com.ls;i++)
   fprintf(fparameter, " %d", global_site[2][i]+1);

   global_rate[3]=500-global_rate[0];
   global_rate[4]=500-global_rate[1];
   fprintf(frandom, ",  %d, %d \n", global_rate[3], global_rate[4]);
*/
   for (i=0;i<com.ls;i++)
...
//////////////////////////////////////////////////////////////////
   ...
   for(j=0; j<com.npatt; j++)
      ...
      if (global_time==1)
      {
         ...
         //x[global_np-global_k*2+2*0+1]=0.08781;
         //x[global_np-global_k*2+2*1+1]=0.89580;
      }
...
/*
         if (global_time>1)           // EM iteration
         {
            ...
            {
                global_c=(global_c1[0]+global_c1[1])/2;
                global_c2[0]=global_c-0.001;
                global_c1[1]=global_c+0.001;

                for (i=0;i<global_np-global_k*3+1;i++) x[i]=global_c2[0]*global_y[i];
                j = ming2t(noisy>2?frub:NULL,&lnL,com.plfun,NULL,x,xb, com.space,e,np, global_k);
                global_c_lnL[0]=lnL;

                for (i=0;i<global_np-global_k*3+1;i++) x[i]=global_c2[1]*global_y[i];
                j = ming2t(noisy>2?frub:NULL,&lnL,com.plfun,NULL,x,xb, com.space,e,np, global_k);
                global_c_lnL[1]=lnL;
            ...
            global_c=(global_c1[0]+global_c1[1])/2;
            for (i=0;i<global_np-global_k*3+1;i++) x[i]=global_c*global_y[i];
            j = ming2t(noisy>2?frub:NULL,&lnL,com.plfun,NULL,x,xb, com.space,e,np, global_k);
         }
*/
       //j = ming2ckw(noisy>2?frub:NULL,&lnL,com.plfun,NULL,x,xb, com.space,e,np, global_k, global_kappa, global_omega, global_model);
...
//         {
//            for (i=0;i<com.npatt;i++)
//            fprintf(fgap, "%f ",com.fhK[global_ig*com.npatt+i]);
//            fprintf(fgap, "\n");
//         }
      }
...
/*
   for (i=0;i<com.npatt;i++)
   {
      ...
      for (j=0;j<global_k;j++)
      {
          if (global_like[j][i]>global_temp0)
          {
             global_temp0=global_like[j][i];
             global_ig=j;
          }
      }
      if (global_time>1 && global_like[global_pose[1][i]][i]<(global_temp0-3))
...
*/
      ...
      for (global_ig=0;global_ig<global_k;global_ig++)
          for (i=0;i<com.npatt;i++)
          {
              ...
              //else
              //global_like[global_ig][i]=global_like[global_ig][i]+log(global_Pi1[global_ig]);
          }
      }
...
/*      global_lnL=0;
      for (i=0;i<com.npatt;i++)
      {
          global_sum=0;
          for (global_ig=0;global_ig<global_k;global_ig++)
          global_sum=global_sum+exp(global_like[global_ig][i]);
          global_lnL=global_lnL+log(global_sum)*com.fpatt[i];
      }
      if (global_time>3)
      {
          if (global_lnL>global_lnL_max)
          global_lnL_max=global_lnL;
          else
          {
             global_iteration=global_time-1;
             global_stop=0;
...
void CountCodons1 (FILE *fout,double fcodonsg[],double fb3x4sg[],double fb4g[],int *miss)
{
   ...
   FOR(ig, com.ngene) {
      FOR(j, com.ns) {
         for(h=0; h<com.npatt; h++) {
            ...
            if(k>1) continue;
            ic=ib[0][0]*16+ib[1][0]*4+ib[2][0];
            fcodonsg[ig*nc+ic]+=com.fpatt[h]*global_z[ig][h]*(com.posG[ig+1]-com.posG[ig])/global_Pi[ig];
         }
      }
...
*/
   ...
   if (global_time>1 && com.Mgene==4)
   CountCodons1(fout, fcodonsg, fb3x4sg, fb4g, &miss);
